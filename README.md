...
**Transforms preservation and stub generation**

- The tool now preserves any `transforms` you declare in your mapping YAML when running `suggest` / `improve`. Your declared transforms will not be overwritten or removed.

- If a mapping references a transform that does not include a package prefix (for example, `OrderID2OrderID`) and that transform is not declared in the `transforms` section, the generator will automatically create a stub implementation for it in a shared `missing_transforms.go` file inside the output package. This helps you compile and iterate quickly while you implement the real transform.

- If you reference a transform with a package prefix (for example `nested_struct.StringToUint`), the generator assumes it is implemented elsewhere and will not generate a stub. Package-prefixed transforms must be available at build time (either implemented in your codebase or reachable via imports).

Example generated stub (placed in the generated package):
...
- Prefer declaring transforms in the `transforms` section of your YAML when you provide an implementation in your codebase (use `func:` or `signature:` as appropriate).
- Use package-qualified transform names when pointing to existing functions in your project (e.g. `my_pkg.TransformName`).
- Let the generator create stubs for quick iteration, then replace the stubs with real implementations in your codebase and add the transform to the YAML transforms list (or call it via package-qualified name).
