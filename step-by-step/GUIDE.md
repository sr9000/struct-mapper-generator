# 📚 Step-by-Step Tutorial: Mastering caster-generator
...
mappings:
  - source: store.Product
    ...
    "121":
      ...
      SKU: SKU
      Name: Name
      PriceCents: Price      # Rename: PriceCents → Price
      Inventory: Stock       # Rename: Inventory → Stock
```
...
You typically don't write `auto` manually — let `suggest` fill it in.
...
5. Re-run `gen` — stubs are removed when real implementations exist
...
- `*T → T` (dereference with nil check)
- `T → *T` (wrap in pointer)
- `*T → *T` (copy pointer or deep copy)
...
When mapping `Order.Items → Order.LineItems`, each `LineItem` might need the `OrderID` from the parent `Order`.
...
`extra` can reference:
- `source: FieldName` — from input struct
- `target: FieldName` — from already-assigned output field
- A `requires` argument name — passed through from parent
...
mappings:
  - source: external.APIResponse
    ...
    generate_target: true    # ← Magic flag
    fields:
      - source: ID
        target: ID
      - source: RawData
...
- `target_pkg/missing_types.go` — Generated type definition
- `generated/casters.go` — Generated caster functions
...
- `store.Order` → `warehouse.Order`
- Handle nested `Customer`, `OrderItem` types
- Type conversions (int64 → uint)
...
- ✅ Analyze packages and discover types
- ✅ Generate mapping suggestions
- ✅ Create caster code
- ✅ Use `121`, `fields`, `ignore`, and `auto` sections
- ✅ Handle type mismatches with transforms
- ✅ Map nested structs and collections
- ✅ Handle pointers and recursive types
- ✅ Pass context between parent and child mappers
- ✅ Generate virtual types on-the-fly
